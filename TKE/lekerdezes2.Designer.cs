...
namespace TKE
{
	public class lekerdezes2 : UserControl
	{
		...
		private ReportDocument rpt2 = new report2();

		private TKE.DataSet ds = new TKE.DataSet();

		public lekerdezes2(mainForm _mainForm)
		{
		...
		private void buttonIndit_Click(object sender, EventArgs e)
		{
			...
			string[] shortDateString = new string[] { "select * from kilometer where datum between '", value.ToShortDateString(), "' and '", dateTime.ToShortDateString(), "'  order by datum " };
			this.da = new SqlDataAdapter(string.Concat(shortDateString), this.myconn);
			this.da.Fill(this.ds, "kilometer");
			this.tableKilometer = this.ds.Tables["kilometer"];
			this.rpt2.SetDataSource(this.ds);
			this.rpt2.SetParameterValue("DatTol", value.ToShortDateString());
			this.rpt2.SetParameterValue("DatIg", dateTime.ToShortDateString());
			this.crystalReportViewer1.ReportSource = this.rpt2;
		}
